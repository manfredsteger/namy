...
"""
content = re.sub(r'(const \[files, setFiles\] = useState<ProcessedFile\[\]>\(\[\]\);)', r'\1' + state_insert, content)

# Extract prescan logic
finalize_method = """
...
# Replace the inner part of handleFilesProcessed
match = re.search(r'// Sort to ensure directories appear before their files.*?setIsProcessing\(false\);', content, re.DOTALL)
if match:
    content = content.replace(match.group(0), "finalizeFiles(processed);")

content = re.sub(r'(const handleFilesProcessed = useCallback\(\(droppedFiles: File\[\]\) => \{)', finalize_method + r'\n  \1', content)

# Add scanDirectory and handleDirectorySelected
direct_mode_methods = """
...
"""
content = re.sub(r'(const handleFilesProcessed = useCallback\(\(droppedFiles: File\[\]\) => \{)', direct_mode_methods + r'\n  \1', content)

# update Dropzone
content = content.replace('<Dropzone onFilesProcessed={handleFilesProcessed} isProcessing={isProcessing} />', '<Dropzone onFilesProcessed={(files) => { setAppMode("upload"); handleFilesProcessed(files); }} isProcessing={isProcessing} onDirectorySelected={handleDirectorySelected} />')
